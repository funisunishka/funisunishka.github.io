...
function loadImages(imageFiles, callback) {
    let images = [];
    let promises = imageFiles.map(src => {
...
function loadResources() {
    return Promise.all([
        new Promise((resolve, reject) => {
            spaceshipImage.src = './others/spaceship.png';
            spaceshipImage.onload = resolve;
            spaceshipImage.onerror = reject;
        }),
        new Promise(resolve => {
            loadImages(planetFiles.map(file => `planets/${file}`), images => {
                planetImages = images;
                resolve();
            });
        }),
        new Promise(resolve => {
            loadImages(asteroidFiles.map(file => `asteroids/${file}`), images => {
                asteroidImages = images;
                resolve();
            });
        })
    ]);
...
function drawMeteor(meteor) {
    ...
    ctx.drawImage(meteor.image, -meteor.width / 2, -meteor.height / 2, meteor.width, meteor.height);
    ctx.restore();
}
...
function createPlanet() {
    ...
    const scale = 1.0 + Math.random() * 1.25; // Adjusted scale for mobile
...
function createMeteors() {
    ...
    for (let i = 0; i < numMeteors; i++) {
        ...
        const meteorImage = asteroidImages[Math.floor(Math.random() * asteroidImages.length)];
        meteors.push({ x, y, width, height, speed: 1 + Math.random() * 2, angle, image: meteorImage });
    }
}
